...
class QueryLimitedContextStorage(ContextStorage):
    ...
    def _init_redis(self):
        ...
        try:
            ...
            # Railway Redis connection - check for Railway environment variables
            redis_url = os.getenv("REDIS_URL")  # Railway provides this
            if redis_url:
                # Parse Redis URL (format: redis://user:pass@host:port/db)
                self.redis_client = redis.from_url(redis_url)
                logger.info(f"Redis backend initialized using REDIS_URL: {redis_url}")
            else:
                ...
                logger.info(
                    f"Redis backend initialized using host: {redis_host}:{redis_port}"
                )
